...
try {
    ...
    if (new_Payment_Mode){
        res.json({message:"Payment Mode Created SuccessFully"})
    } 
} catch (error) {
    console.log(error);
...
try {
    const {id,payment_mode_name}= req.body;
    const existingpayment_mode = await Payment_Mode.findByPk(id);
...
const show = async(req,res)=>{
    try {
        const {id} = req.params;
        const payement_mode = await Payment_Mode.findByPk(id);
        if (payement_mode){
